...
void loop() {
  ...
  //Only runs if the network/lockout switch is down
  if(netState) {
    if (Serial.available()>0) {
      String updateData = Serial.readString();
      int split = updateData.indexOf(":");
      updateFromNet(updateData.substring(0,split),updateData.substring(split+1));
      Serial.println(updateData.substring(0,split));
      Serial.println(updateData.substring(split+1));
      updateOutputs();
    }
  }
...
void updateFromNet(String type, String value) {
  char topic[type.length()+1];
  type.toCharArray(topic,type.length()+1);
  Serial.println("recieved smth");
  if (netState) {
    
    if (strcmp(topic,"channel-a-amplitude")==0){
      ampAState = value.toInt();
      Serial.println("ran");
    }
    
    if (strcmp(topic,"channel-a-temperature")==0) {
      //Serial.println("Change in temp");
      tempAState = value.toInt();
    }

    if (strcmp(topic,"channel-b-amplitude")==0){
      ampBState = value.toInt();
      Serial.println("ran");
    }
    
    if (strcmp(topic,"channel-b-temperature")==0) {
      //Serial.println("Change in temp");
      tempBState = value.toInt();
    }
    
    if (strcmp(topic,"ac-1-setting")==0) {
      ac1State = value.toInt();
    }
    if (strcmp(topic,"ac-2-setting")==0) {
      ac2State = value.toInt();
    }
    if (strcmp(topic,"ac-3-setting")==0) {
      ac3State = value.toInt();
    }
    if (strcmp(topic,"ac-4-setting")==0) {
      ac4State = value.toInt();
    }
  
    //Custom shift Routines
    //duration, start amplitude, end amplitude, start temp, end temp
    if (strcmp(topic,"sunrise")==0 && shiftState) {
      StripA.shift(600000L, 0, 240, 240, 180);
    }
    if (strcmp(topic,"sunset")==0 && shiftState) {
